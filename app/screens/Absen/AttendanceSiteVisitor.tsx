...
// Types
type WorkMode = 'Site Visitor' | 'WFO';
...
type AttendanceSiteVisitorNavigationProp = StackNavigationProp<RootStackParamList, 'AttendanceSiteVisitor'>;

interface AttendanceSiteVisitorProps {
    navigation: AttendanceSiteVisitorNavigationProp;
}

const AttendanceSiteVisitor: React.FC<AttendanceSiteVisitorProps> = ({ navigation }) => {
    ...
    const [workMode, setWorkMode] = useState<WorkMode>('Site Visitor');
        ...
        try {
            ...
            if (!isGranted) {
                Alert.alert(
                    ...
                    'Aplikasi memerlukan izin akses kamera untuk foto selfie saat absensi. Silakan aktifkan di pengaturan aplikasi.',
                    [
                        { text: 'Batal', style: 'cancel' },
                        {
                            text: 'Buka Pengaturan',
                            onPress: () => {
                                // Untuk membuka settings (optional, perlu linking)
    ...
    const handleCheckIn = async (): Promise<void> => {
        ...
        try {
            ...
            // Append data - gunakan default Site Visitor untuk check-in
            formData.append('work_mode', 'Site Visitor');
            formData.append('notes', notes || 'Absen Site Visitor - Check-in');

            // PENTING: Kirim null atau kosongkan untuk Site Visitor (tidak perlu koordinat)
            // Jangan kirim string '0', biarkan kosong atau kirim null
            formData.append('address', 'Site Visitor - Tidak Memerlukan Lokasi');
    ...
    // Proses absensi checkout (dengan modal pilih Site Visitor/WFO)
    const processCheckout = async (): Promise<void> => {
        ...
        try {
            ...
            // Untuk Site Visitor, kirim koordinat dummy (0,0) karena tidak validasi lokasi
        ...
        >
            <View style={styles.modalOverlay}>
                <View style={styles.modalContent}>
                    ...
                    <TouchableOpacity
                        style={[styles.modeButton, workMode === 'Site Visitor' && styles.modeButtonActive]}
                        onPress={() => setWorkMode('Site Visitor')}
                    >
                        <FeatherIcon name="home" size={24} color={workMode === 'Site Visitor' ? '#fff' : COLORS.primary} />
                        <Text
                            style={[styles.modeButtonText, workMode === 'Site Visitor' && styles.modeButtonTextActive]}
                        >
                            Work From Home
                        </Text>
                    </TouchableOpacity>
            ...
            <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
                ...
                >
                    ...
                    <View style={styles.header}>
                        ...
                        <Text style={[FONTS.h3 as TextStyle, styles.headerTitle]}>Absensi di Luar Kantor</Text>
                        ...
                        <View style={styles.statusCard}>
                            ...
                            {status.attendance?.work_mode && (
                                <View style={styles.workModeInfo}>
                                    <FeatherIcon
                                        name={status.attendance.work_mode === 'Site Visitor' ? 'home' : 'briefcase'}
                                        size={16}
                                        color={COLORS.primary}
                                    />
                                    <Text style={[FONTS.fontSm as TextStyle, styles.workModeText]}>
                                        {status.attendance.work_mode === 'Site Visitor'
                                            ? 'Work From Home'
                                            : 'Work From Office'}
                                    </Text>
                                </View>
                    ...
                    {(status?.can_check_in || status?.can_check_out) && (
                        <View style={styles.notesSection}>
                            ...
                            <TextInput
                                ...
                                // placeholder="Contoh: Site Cibitung, Proyek BSD, Client Jakarta, dll."
                            ...
                            />
                            <Text style={[FONTS.fontXs as TextStyle, styles.notesHelper]}>
                                Jelaskan lokasi atau kegiatan Anda hari ini
                            </Text>
                        </View>
                    )}
                    ...
                    <View style={styles.infoCard}>
                        ...
                        <Text style={[FONTS.fontXs as TextStyle, styles.infoText]}>
                            Absensi di Luar Kantor tidak memerlukan validasi lokasi. Cukup ambil foto selfie dan pilih
                            mode kerja Anda.
                        </Text>
                    </View>
                </ScrollView>
...
export default AttendanceSiteVisitor;