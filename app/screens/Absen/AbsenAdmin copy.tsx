...
const AbsenAdmin: React.FC<AbsenAdminProps> = ({ navigation }) => {
    ...
    const ALLOWED_RADIUS = 50; // 10 meters
        ...
        try {
            ...
            if (!isGranted) {
                Alert.alert(
                    ...
                    'Aplikasi memerlukan izin akses kamera untuk foto selfie saat check-in.',
                    [{ text: 'OK' }]
                );
            }
    ...
    const openCamera = async (): Promise<void> => {
        ...
        try {
            const result = await ImagePicker.launchCameraAsync({
                ...
                aspect: [3, 4],
                quality: 0.8,
                cameraType: ImagePicker.CameraType.front, // Gunakan kamera depan untuk selfie
            });
            ...
            if (!result.canceled && result.assets && result.assets.length > 0) {
                ...
                Toast.show({
                    type: 'success',
                    text1: 'Foto Berhasil',
                    text2: 'Foto selfie berhasil diambil'
                });
            }
        } catch (error) {
    ...
    const handleCheckin = async (): Promise<void> => {
        ...
        try {
            ...
            console.log("Check-in with photo");
            ...
            if (response.ok && responseData.success) {
                ...
                setCapturedPhoto(null); // Reset foto setelah berhasil
    ...
    const handleCheckout = async (): Promise<void> => {
        if (!isInRadius) {
            Alert.alert(
                'Diluar Radius',
                `Anda berada ${Math.round(distance || 0)}m dari kantor.`,
                [{ text: 'OK' }]
            );
            return;
        }
        ...
        try {
            const formBody = new URLSearchParams({
                ...
                address: branchOffice?.address || 'Kantor Pusat',
            }).toString();
            ...
            if (response.ok && responseData.success) {
                ...
                Toast.show({
                    type: 'error',
                    text1: 'Error',
                    text2: responseData.message || 'Check-out gagal',
                });
            }
        } catch (error) {
                        ...
                        {!hasCheckedIn ? (
                            ...
                            <View style={styles.contentContainer}>
                                ...
                                {!hasCheckedOut && (
                                    <View style={{ marginTop: 20, width: '100%' }}>
                                        <ButtonLight
                                            color={isInRadius ? COLORS.danger : COLORS.secondary}
                                            title={'Check Out'}
                                            onPress={handleCheckout}
                                            disabled={!isInRadius || isLoading}
                                        />
                                    </View>
                                )}
