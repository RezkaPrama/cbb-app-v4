...
export default function ArticleSearchModal({ visible, onClose, onPick }: Props) {
  ...
  useEffect(() => {
    ...
    apiService.searchArticles(q).then(async (res) => {
      ...
      // 1 request batch untuk SEMUA produk hasil pencarian sekaligus (bukan
      // N request per produk) — jauh lebih ringan buat shared hosting.
      const productIds = products.map((p: ArticleSearchResult) => p.id);
      const stockRes = await apiService.checkStockMultiple(productIds);
      if (!cancelled && stockRes.success && stockRes.data) {
        const map: Record<number, number> = {};
        for (const id of productIds) {
          map[id] = stockRes.data[id]?.available_pcs ?? 0;
        }
        setStockMap(map);
      }
    });
