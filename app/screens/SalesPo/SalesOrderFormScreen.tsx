...
export default function SalesOrderFormScreen({ navigation }: Props) {
  ...
  const [tanggalFaktur] = useState<string>(todayISO());
  ...
  useEffect(() => {
    apiService.generateSalesOrderCode().then((res) => {
      ...
      if (res.success && res.data) setPoCode(res.data.code);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    apiService.searchSalesOrderCustomers(q).then((res) => {
      ...
      if (res.success && res.data) setCustomerResults(res.data.data);
    });
  ...
  const handleSelectCustomer = async (c: CustomerSearchResult) => {
    ...
    if (res.success && res.data) {
      ...
      setCustomerDiscounts(res.data.discounts || {});
      // Auto-isi diskon 1 sesuai jenis transaksi yang sedang dipilih
      const discountValue = res.data.discounts?.[jTrans] ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    setSuccessResult({ code: res.data.code });
  };
          ...
          <View style={styles.card}>
            ...
            <View style={styles.row2}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <View style={styles.readonlyBox}>
                  <Text style={styles.readonlyBoxText}>{tanggalFaktur}</Text>
                </View>
              </View>
            </View>
